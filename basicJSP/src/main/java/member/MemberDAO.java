...
public class MemberDAO {
	...
	public int checkMemberList(String id) {
		for(int i = 0; i < list.size(); i++) {
			if(list.get(i).getId().equals(id) && i != log) {
				return i;
			}
		}
		return -1;
	...
	public void addMemberList(String id,String pw,String name,boolean gender) {
		list.add(new Member(id, pw, name, gender));
	}
	
	public void removeMemberList(int idx) {
		list.remove(idx);
	}
